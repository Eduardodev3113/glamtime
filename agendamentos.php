...
<body class="bg-light">
  ...
  <main class="container">
    <?php if ($flash): ?>
      <div class="alert alert-success"><?= htmlspecialchars($flash) ?></div><?php endif; ?>
    ...
    <table class="table table-hover bg-white shadow-sm">
      ...
      <tbody><?php foreach ($lista as $a): ?>
          <tr>
            ...
            <td><span
                class="badge <?= $badges[$a['status']] ?? 'bg-secondary' ?>"><?= $rotulos[$a['status']] ?? $a['status'] ?></span>
            </td>
            <td>
              <?php if ($a['status'] === 'agendado'): ?>
                <a href="agendamento_editar.php?id=<?= (int) $a['id'] ?>" class="btn btn-outline-primary btn-sm">Editar</a>
                <a href="agendamento_agendar_novamente.php?id=<?= (int) $a['id'] ?>"
                  class="btn btn-outline-success btn-sm">Agendar novamente</a>
                <form method="post" action="agendamento_cancelar.php" class="d-inline">
                  <input type="hidden" name="id" value="<?= (int) $a['id'] ?>">
                  <input type="hidden" name="csrf"
                    value="<?= htmlspecialchars($_SESSION['csrf'] ?? ($_SESSION['csrf'] = bin2hex(random_bytes(32)))) ?>">
